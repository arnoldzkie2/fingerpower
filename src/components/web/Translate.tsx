...
const Translate: React.FC<TranslateProps> = ({ locale, scroll, isOpen }) => {
    ...
    return (
        <select className={`py-2 px-1 text-center text-sm bg-transparent border-0 cursor-pointer border-b ${scroll || isOpen ? 'border-primary text-gray-700' : 'border-gray-200 text-white focus:bg-primary'} appearance-none focus:outline-none focus:ring-0 focus:border-gray-200 outline-none`} value={locale} onChange={handleTranslation}>
